...
def plotOneModel(pred1, true, model):
    fig = plt.figure()
    ax = fig.add_subplot(111)
    plt.plot(range(len(pred1)), pred1,':', label=model)
    # plt.plot(range(len(pred2)), pred2, label='pred2')
    plt.plot(range(len(true)), true, label='actual')
    plt.xlabel('时间')
    plt.ylabel('积涝水位深度(mm)')
...
def build_model():
    ...
    print(train_X.shape)  # （-1，8,2）
    print(train_label.shape)  # (-1,1)
    if (train == True):
    ...
    if (train == False):
        ...
        model3 = load_model('lstm.h5')
        model4 = load_model('cnn-lstm.h5')
        model5 = load_model('gru.h5')
        ...
        prediction4 = model4.predict(test_X.reshape(-1, 4, 4, 1))
        prediction5 = model5.predict(test_X)
        scaled_prediction1 = prediction1 * std_y + mean_y
        scaled_prediction2 = prediction2 * std_y + mean_y
        scaled_prediction3 = prediction3 * std_y + mean_y
        scaled_prediction4 = prediction4 * std_y + mean_y
        scaled_prediction5 = prediction5 * std_y + mean_y
        ...
        plotOneModel(scaled_prediction1, scaled_test_label, "TCN-LSTM")
        plotOneModel(scaled_prediction2, scaled_test_label, "TCN")
        plotOneModel(scaled_prediction3, scaled_test_label, "LSTM")
        plotOneModel(scaled_prediction4, scaled_test_label, "CNN-LSTM")
        plotOneModel(scaled_prediction5, scaled_test_label, "GRU")
